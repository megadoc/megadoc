# tinydoc's primary UI plugin

This plugin is the default React UI provided by tinydoc.

## Features

### disqus integration

The UI provides integration with [disqus](https://disqus.com/) to allow for readers to leave comments on certain pages.

## Configuration

Note that the config for this plugin does not need to be scoped under `ui` like the rest of the plugins are scoped, simply define the parameters on
the exported module `module.exports`.
...
module.exports = {
   ...
   * @property {String[]}
   *
   * An array of paths to custom .js scripts to include in the resulting
   * html file.
   */
  scripts: [],
   ...
   * @property {String[]}
   *
   * This is usually handled internally by the included plugins. Files
   * specified here will be treated as "plugins" to the UI and are handled
   * in a special manner.
   *
   * Any script defined here is expected to call `tinydoc.use()` when it is
   * loaded so that the PluginManager knows the plugin is ready to go and
   * can start the application afterwards.
   */
  pluginScripts: [],
