...
module.exports = function(config, compiler, database, done) {
  ...
  if (config.styleOverrides) {
    if (typeof config.styleOverrides === 'object') {
      compiler.assets.addStyleOverrides(config.styleOverrides);
    }
    else {
      console.warn(
        "Deprecated: assign styleOverrides directly instead of pointing to a " +
        ".less file."
      );
    }
  }
  ...
  if (config.stylesheet) {
    compiler.assets.addStyleSheet(compiler.utils.getAssetPath(config.stylesheet));
  }
