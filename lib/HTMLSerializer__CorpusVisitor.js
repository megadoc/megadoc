  ...
  function NodeURI(node) {
    ...
    if (node.type === 'DocumentEntity') {
      ...
      return (
        ...
        // what's happening here is that documents that nest other documents
        // beneath them should be placed inside /path/to/document/index.html
        // instead of /path/to/document.html and that is because there may be
        // a sibling document (that is, at this level in the tree) that has the
        // same name INCLUDING the extension (like `tinymce` and `tinymce.html`)
        // in which case we'll have conflicts since both documents will point to
        // /path/to/tinymce.html
        //
        // This way, for a document structure that looks like this:
        //
        //     [
