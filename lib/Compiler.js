...
function Compiler(config) {
  assert(!!config && typeof config === 'object',
    "Compiler must be instantiated with a config object.");
   ...
   */
  this.corpus = Corpus.Corpus({ strict: config.strict, debug: config.debug });
  ...
  this.corpus.visit({
    Document: aliasNode,
    DocumentEntity: aliasNode
  });

  function aliasNode(node) {
    if (alias[node.uid]) {
      that.corpus.alias(node.uid, alias[node.uid]);
    }
  }
